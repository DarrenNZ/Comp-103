...
public class GeneralTree
{
    ...
    public void addNode(String newName, String parentName) {
        ...
        if (root == null)
            ...
            GeneralTreeNode temp = this.root;
            this.root = new GeneralTreeNode(newName);
            this.root.addChild(temp);   
        }
        else {//if has parent and isnt root 

            GeneralTreeNode parent = root.findNode(parentName);//returns null
            if(parent == null)
                UI.println("oh no");

            UI.println("**parent name " + parent.getName());
            GeneralTreeNode temp = new GeneralTreeNode(newName);
            //temp.setLocation();
            //temp.redrawSubtree();
            parent.addChild(temp);

            //UI.println("**child name " + temp.getName());
        }
    ...
    public void removeNode(String targetName) {
        ...
        //Set <GeneralTreeNode> children = new HashSet <GeneralTreeNode>();
        GeneralTreeNode child = root.findNode(targetName);
        //GeneralTreeNode grandParent = null;
        if (child != null){
            //grandParent = child.getParent();
            //children = grandParent.getChildren();
            child.remove();
            child = new GeneralTreeNode(null);
           // root.redrawSubtree();
        }
