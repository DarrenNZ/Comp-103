...
public class Images implements Iterable<String>
{
    ...
    public void moveCursorLeft() {  
        ...
        // while the node before the cur    sor has not been found yet, keep advancing
    ...
    public void addImageAfter(String imageFileName) {
        ...
        if (head != null) {
            temp = new ImageNode(imageFileName, cursor); //next node
            cursor = head = temp; // not head = head??

        } else{
            temp = new ImageNode(imageFileName, null);
            cursor = head = temp;
    ...
    public void remove() {
        if (head.count() > 0){
            if (head.count() > 0 && head.count() < 2){
                ...
                ImageNode temp = head.nodeBefore(cursor);
                cursor.removeNodeUsingPrevious(temp);

                /**ImageNode temp = head.nodeBefore(cursor);
                ImageNode temp1 = temp.getNext().getNext();
                temp.setNext(temp1);
