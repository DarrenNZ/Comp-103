...
public class FastFood{
    ...
    public FastFood() {
        ...
        /*# YOUR CODE HERE */
        //add items to customers order at the head of the que, when complete, deliver order, take off que, add price of order to balance
    ...
    public void addItem(String item) {
        ...
        if(orders.size() > 0){
            ...
            if (orders.peek().addItemToOrder(item) == true){
                this.balance+= orders.peek().getPrice();
            }
            else{
                this.balance-= orders.peek().getPrice();
                UI.drawString("WARNING!, item is not in the order - score has been deducted!!", 300, 20);
                UI.sleep(4000);
            }
        }
    }
    ...
    public void deliverOrder() {
        /*# YOUR CODE HERE */
        if(orders.peek().isReady() == true){
            Order purchase = new Order();
            purchase = orders.poll();
            balance += purchase.getPrice();

            //remove from que, getprice, update total balance else prints warning message;}

        }
        else{
            UI.drawString("WARNING!, this order is not ready yet!!", 300, 20);
            UI.sleep(4000);
        }
    }
